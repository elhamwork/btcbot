...
The long version. Start at [the front page](../README.md) if you only want to
know how it is doing.

This file is the record of the original 14-day study -- the one that found no
edge, and the look-ahead bug that had faked one. The model running today is a
later, different thing, built on 63 days; its own reasoning lives in the
header of `check.py` and its literature review in
[`../results/reports/literature_review.md`](../results/reports/literature_review.md).
